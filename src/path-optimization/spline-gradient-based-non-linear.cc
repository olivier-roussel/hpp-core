...
namespace hpp {
  namespace core {
    ...
    using constraints::function::OfParameterSubset;
    typedef std::vector<NumericalConstraints_t> NumericalConstraintss_t;

    namespace pathOptimization {
      template <typename T>
      void insertIfNotPresent (std::vector<T>& out, const std::vector<T>& in)
      {
        std::vector<T>::const_iterator last = out.end();
        for (std::vector<T>::const_iterator _in = in.begin(); _in != in.end(); ++_in) {
          if (last == std::find(v.begin(), last, *_in))
            v.push_back (v.end(), *_in);
        }
      }
      ...
      constraints::ImplicitPtr_t makeVaryingRHSConstraint (const constraints::ImplicitPtr_t nc,
          const std::size_t start, const std::size_t end)
      {
        // size_type nCfg = nc->function().inputSize(),
                  // nDof = nc->function().inputDerivativeSize();
      ...
      constraints::ImplicitPtr_t makeConstantRHSConstraint (
          const constraints::ImplicitPtr_t nc, const std::size_t N, const std::size_t start)
      {
        size_type nCfg = nc->function().inputSize(),
                  nDof = nc->function().inputDerivativeSize();
        DifferentiableFunctionPtr_t f = OfParameterSubset::create (
            nc->functionPtr(), N * nCfg, N * nDof,
            segment_t (i * nCfg, nCfg), segment_t (i * nDof, nDof));

        return constraints::Implicit::create (f, nc->comparisonTypes(),
            nc->rightHandSize());
      }
	...
	return shPtr;
      ...
      template <int _PB, int _SO>
      void SplineGradientBased<_PB, _SO>::getNumericalConstraints
      (const PathVectorPtr_t& init, const Splines_t& splines,
       NumericalConstraintss_t& numCsPerPath,
       NumericalConstraintss_t& numCsPerWaypoint) const;
      {
        assert (init->numberPaths() == splines.size() && ss.size() == splines.size());
        assert (numCsPath.size()+1 == numCsPerWaypoint.size());
        for (std::size_t i = 0; i < splines.size(); ++i) {
          addProblemConstraintOnPath (init->pathAtRank(i), i, splines[i],
              numCsPerPath[i], numCsPerWaypoint[i], numCsPerWaypoint[i+1]);
        }
      }

      template <int _PB, int _SO>
      void SplineGradientBased<_PB, _SO>::getNumericalConstraintsOnPath
      (const PathPtr_t& path, const size_type& idxSpline, const SplinePtr_t& spline,
       NumericalConstraints_t& numCsPath,
       NumericalConstraints_t& /*numCsStart*/,
       NumericalConstraints_t& /*numCsEnd  */) const;
      {
        ...
        if (cs) {
          ...
          if (cp) {
            ...
            insertIfNotPresent (numCsPath, hs.numericalConstraints());
          }
        }
      }

      template <int _PB, int _SO>
      void SplineGradientBased<_PB, _SO>::makeConstraintsOnProblemVariable
      (NumericalConstraintss_t& numCsPerPath,
       NumericalConstraintss_t& numCsPerWaypoint) const;
      {
        for (std::size_t i = 0; i < numCsPerPath.size(); ++i) {
          NumericalConstraints_t& numCsPath = numCsPerPath[i];
          for (std::size_t j = 0; j < numCsPath.size(); ++j) {
            // If it has a verying right hand side
            constraints::ImplicitPtr_t& nc = numCsPath[j];
            if (nc->constantRightHandSide()) {
              // Find all consecutive equal constraints
              std::size_t last;
              for (last = i+1; last < numCsPerPath.size(); ++last) {
                NumericalConstraints_t& otherNumCsPath = numCsPerPath[last];
                NumericalConstraints_t::iterator _otherNc = 
                  std::find (otherNumCsPath.begin(), otherNumCsPath.end(), nc);
                if (_otherNc == otherNumCsPath.end()) break;
                otherNumCsPath.erase(_otherNc);
              }
              // Same constraint from spline i to spline last-1
              // Convert it to a constraint taking all variable as input.
              nc = makeVaryingRHSConstraint (nc, i, last-1);
            } else {
              // Convert it to a constraint taking all variable as input.
              nc = makeConstantRHSConstraint (nc, numCsPerPath.size(), i);
            }
          }
        }
      }
